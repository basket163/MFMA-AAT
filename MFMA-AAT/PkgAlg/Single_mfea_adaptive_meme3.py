...
class AdaptiveHelper(object):
    def __init__(self, fit_func_list, pa=0.901):
        self.pa = pa
        #包含一组SkillRecord
...
def fx(query, arg_vec, help_task_idx):
    #对多余的fx函数，初始化为f1，实际不会使用
    if help_task_idx > len(query.user_group) - 1:
        return f1(query, arg_vec)

    mask = query.user_group[help_task_idx]
...
def f2(query, arg_vec):
    return fx(query, arg_vec, 0)

def f3(query, arg_vec):
    return fx(query, arg_vec, 1)

def f4(query, arg_vec):
    return fx(query, arg_vec, 2)

def f5(query, arg_vec):
    return fx(query, arg_vec, 3)

def f6(query, arg_vec):
    return fx(query, arg_vec, 4)
...
def memetic(query, first_phen, first_val, population, fit_list):
    ...
    #meme_num = int(query.pop_size / 2 * query.pl)
    #if meme_num < 1:
    #    meme_num = 1
    #meme_range = np.arange(query.pop_size)
    
    # user=50，模因算法3不行， user > 100，模因算法3很好
    if query.user_count / query.cell_id_max > 5:
        #为降低memetic时间，只对排第一的个体做memetic search
        meme_num = 1
    else:
        #为降低memetic时间，只对排第一的个体做memetic search
        meme_num = 2
    ...
    for i in meme_idx:
        ...
        #start_hill = util.sta_show_now_time_msg(msg='begin a hill climb ')
        # hill climbing local search
        start_num = int(meme_num*query.pl)
        start_num = min(start_num, meme_num)
        if meme_num > 1:
            start_num = 1
        ...
        #util.sta_show_used_time_msg(start_hill, msg='')
...
def run_Single_MFEA_adaptive_meme3(query):
    #包括main和helper，f1是main
    fit_func_list = [f1, f2, f3, f4, f5, f6]
    #start = util.sta_show_now_time_msg(msg='begin a MFEA_adaptive_meme ')
    run_single_mfea_adapt_meme3(query, fit_func_list)
    #util.sta_show_used_time_msg(start, msg='')

def run_single_mfea_adapt_meme3(query, fit_func_list):
    ...
    fit_func_record = AdaptiveHelper(helper_func_list, pa=query.pa)
    ...
    mutate = util.mutate_single
    if query.mutate_indi == 2:
        mutate = util.mutate
    elif query.mutate_indi == 3:
        mutate = util.mutate_uniform
    elif query.mutate_indi == 4:
        mutate = util.mutate_swap
    else:
        mutate = util.mutate_single
    ...
    for t in iterator:
        ...
        for i in range(0, N, 2):
            ...
            if query.cross_indi == 2:
                crossover = util.x_two_point
                #print("x_two_point")
            elif query.cross_indi == 3:
                crossover = util.x_uniform_point
                #print("x_uniform_point")
            elif query.cross_indi == 4:
                crossover = util.x_complex_two_uniform
            else:
                crossover = util.x_single_point
                #print("x_single_point")
            #crossover = util.
